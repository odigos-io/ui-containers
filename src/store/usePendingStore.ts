...
const itemsAreEqual = (item1: PendingItem, item2: PendingItem) => {
  ...
  const idsEqual =
    typeof item1.entityId === 'string' && typeof item2.entityId === 'string'
      ...
      ? item1.entityId.namespace === item2.entityId.namespace &&
        item1.entityId.name === item2.entityId.name &&
        item1.entityId.kind === item2.entityId.kind
      : !item1.entityId && !item2.entityId
...
export const usePendingStore = create<StoreState>((set, get) => ({
  ...
  addPendingItems: (arr) =>
    set((state) => ({
      pendingItems: state.pendingItems.concat(
        arr.filter((addItem) => !state.pendingItems.some((existingItem) => itemsAreEqual(existingItem, addItem)))
      ),
    })),
  removePendingItems: (arr) =>
    set((state) => ({
